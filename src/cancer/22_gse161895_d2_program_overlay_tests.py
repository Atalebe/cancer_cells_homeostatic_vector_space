...
def main() -> None:
    ...
    out_dir = REPO_ROOT / cfg["results_dir"] / "d2_program_overlay_tests"
    ...
    total_counts = np.asarray(mat_d2.sum(axis=0)).ravel().astype(float)
    mito_mask = gene_df["is_mito"].fillna(False).infer_objects(copy=False).to_numpy(dtype=bool)
    non_mito_mask = ~mito_mask

    mito_counts = np.asarray(mat_d2[mito_mask, :].sum(axis=0)).ravel().astype(float)
    non_mito_counts = np.asarray(mat_d2[non_mito_mask, :].sum(axis=0)).ravel().astype(float)
    ...
    score_df = pd.DataFrame({
        ...
        "non_mito_counts": non_mito_counts,
    ...
    score_df.to_csv(out_dir / "d2_cell_program_scores.csv", index=False)
    ...
    summary_df = pd.DataFrame(rows)[[
        "metric", "group_a", "group_b", "n_a", "n_b", "median_a", "median_b",
        "delta_median_a_minus_b", "u_statistic", "p_value"
    ]]
    summary_df.to_csv(out_dir / "d2_subdomain_program_overlay_tests.csv", index=False)

    with open(out_dir / "d2_program_overlay_summary.json", "w", encoding="utf-8") as f:
