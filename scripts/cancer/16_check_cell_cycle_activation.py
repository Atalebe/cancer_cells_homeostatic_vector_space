...
def main() -> None:
    ...
    g1s_genes = gs["cell_cycle"]["g1_s"]
    g2m_genes = gs["cell_cycle"]["g2_m"]

    g1s = score_gene_set_mean_z(x, g1s_genes)
    g2m = score_gene_set_mean_z(x, g2m_genes)
    cell_cycle_total = 0.5 * (g1s + g2m)

    overlay = pd.DataFrame(
        {
            "cell_id": x.index,
            "cell_cycle_g1s_score": g1s.values,
            "cell_cycle_g2m_score": g2m.values,
            "cell_cycle_total_score": cell_cycle_total.values,
        }
    )

    merged = state.merge(overlay, on="cell_id", how="left")

    overlay_path = tables_dir / "cell_cycle_overlay_table.csv"
    write_table(merged, overlay_path)

    group_summary = (
        merged.groupby("population_label")[
            ["cell_cycle_g1s_score", "cell_cycle_g2m_score", "cell_cycle_total_score", "H", "S", "M", "R"]
        ]
        .mean()
        .round(4)
        .reset_index()
    )
    group_summary_path = tables_dir / "cell_cycle_overlay_group_summary.csv"
    ...
    corr_path = tables_dir / "cell_cycle_overlay_correlations.csv"
    write_table(corr, corr_path)

    summary = {
        "dataset_id": cfg["dataset"]["dataset_id"],
        "g1s_match_report": gene_set_match_report(x, g1s_genes),
        "g2m_match_report": gene_set_match_report(x, g2m_genes),
        "outputs": {
    ...
    with open(tables_dir / "cell_cycle_overlay_summary.json", "w", encoding="utf-8") as fh:
