...
def main() -> None:
    ...
    glycolysis_genes = gs["metabolic"]["glycolysis"]
    oxphos_genes = gs["metabolic"]["oxphos"]

    glycolysis = score_gene_set_mean_z(x, glycolysis_genes)
    oxphos = score_gene_set_mean_z(x, oxphos_genes)
    metabolic_balance = glycolysis - oxphos

    overlay = pd.DataFrame(
        {
            "cell_id": x.index,
            "glycolysis_score": glycolysis.values,
            "oxphos_score": oxphos.values,
            "metabolic_balance_score": metabolic_balance.values,
        }
    )

    merged = state.merge(overlay, on="cell_id", how="left")

    overlay_path = tables_dir / "metabolic_overlay_table.csv"
    write_table(merged, overlay_path)

    group_summary = (
        merged.groupby("population_label")[
            ["glycolysis_score", "oxphos_score", "metabolic_balance_score", "H", "S", "M", "R"]
        ]
        .mean()
        .round(4)
        .reset_index()
    )
    group_summary_path = tables_dir / "metabolic_overlay_group_summary.csv"
    ...
    corr_path = tables_dir / "metabolic_overlay_correlations.csv"
    write_table(corr, corr_path)

    summary = {
        "dataset_id": cfg["dataset"]["dataset_id"],
        "glycolysis_match_report": gene_set_match_report(x, glycolysis_genes),
        "oxphos_match_report": gene_set_match_report(x, oxphos_genes),
        "outputs": {
    ...
    with open(tables_dir / "metabolic_overlay_summary.json", "w", encoding="utf-8") as fh:
